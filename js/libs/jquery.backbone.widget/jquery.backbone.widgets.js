		...
		show: function() {
			...
			collection.each(function(model) {
				...
				if ( (v.indexOf(str) == 0 || (options.noCase && v.toLowerCase().indexOf(str_lower) == 0)) || 
					(v2.indexOf(str) == 0 || (options.noCase && v2.toLowerCase().indexOf(str_lower) == 0)) )
					result.push(model);
			});
			...
			for (var i = 0; i < result.length; i++) {
				...
				var html = '<span class="emailInfo">&lt;' + model.getFirstEmailAddress() + '&gt;</span>' + model.get("contactFirstName") + " " + model.get("contactLastName");// '<strong>'+found.substr(0, str.length)+'</strong>'+found.substr(str.length);
